{{-- resources/views/editor/projects/table.blade.php --}}
<div class="py-6">
                    ...
                    @foreach($projects as $project)
                        <tr class="hover:bg-slate-50 transition" data-project-id="{{ $project->id }}">
                            ...
                            <td class="px-6 py-4 text-sm text-right space-x-2">
                                    <a href="{{ route('editor.projects.edit', $project) }}" class="text-emerald-600 hover:text-emerald-800 font-medium transition">Edit</a>
                                    <button type="button" onclick="deleteProject({{ $project->id }}, 'editor')" class="text-red-600 hover:text-red-800 ml-3">Hapus</button>
                                    <form method="POST" action="{{ route('editor.projects.update', $project) }}" class="inline-block ml-3">
                                        @csrf
                                        @method('PATCH')
...
<script>
function deleteProject(projectId, role = 'admin') {
    if (!confirm('Hapus proyek ini?')) return;

    const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
    const route = role === 'editor' ? `/editor/projects/${projectId}` : `/admin/projects/${projectId}`;
