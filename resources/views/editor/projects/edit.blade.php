...
<div class="max-w-4xl mx-auto py-10">
    <div class="bg-white rounded-lg shadow p-6">
        <h2 class="text-xl font-bold mb-4">Edit Proyek (Editor)</h2>

        @if(session('success'))
            <div class="mb-4 text-green-700 bg-green-100 px-4 py-2 rounded">{{ session('success') }}</div>
        @endif

        <form method="POST" action="{{ route('editor.projects.update', $project) }}">
            @csrf
            @method('PATCH')
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('editor.projects.index') }}" class="px-4 py-2 bg-slate-200 rounded">Batal</a>
            </div>
        </form>
    </div>
</div>
