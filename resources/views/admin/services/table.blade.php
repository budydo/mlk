{{-- resources/views/admin/services/table.blade.php --}}
<div class="py-6">
    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <form method="GET" action="{{ route($routePrefix . '.index') }}" class="flex-1 flex gap-3 items-center">
            <input type="text" name="q" value="{{ old('q', $q ?? '') }}" placeholder="Cari layanan..." class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent">
        ...
        {{-- Tombol tambah layanan menuju halaman create --}}
        <div>
            <a href="{{ route($routePrefix . '.create') }}" class="px-4 py-2 bg-emerald-600 text-white rounded-md">+ Tambah Layanan</a>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        @if($services->count() > 0)
            <table class="min-w-full divide-y divide-slate-200">
                ...
                <tbody class="divide-y divide-slate-200">
                    @foreach($services as $service)
                        <tr class="hover:bg-slate-50 transition" data-service-id="{{ $service->id }}">
                            <td class="px-6 py-4 text-sm font-medium text-slate-900">{{ $service->title }}</td>
                            <td class="px-6 py-4 text-sm text-slate-600">{{ $service->slug }}</td>
                            <td class="px-6 py-4 text-sm">
                                {{-- Form PATCH sederhana: tombol menampilkan badge dan mengirimkan nilai kebalikan supaya toggle terjadi --}}
                                <form method="POST" action="{{ route($routePrefix . '.update', $service) }}" class="inline-block">
                                    @csrf
                                    @method('PATCH')
                                    <input type="hidden" name="title" value="{{ $service->title }}">
                                    <input type="hidden" name="slug" value="{{ $service->slug }}">
                                    {{-- kirim nilai yang berlawanan untuk toggle --}}
                                    <input type="hidden" name="is_published" value="{{ $service->is_published ? '0' : '1' }}">
                            ...
                            <td class="px-6 py-4 text-sm text-right space-x-2">
                                <a href="{{ route($routePrefix . '.edit', $service) }}" class="text-emerald-600 hover:text-emerald-800 font-medium transition">Edit</a>
                                <button type="button" onclick="deleteService({{ $service->id }}, '{{ route($routePrefix . '.index') }}')" class="text-red-600 hover:text-red-800 ml-3">Hapus</button>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
            ...
            <div class="px-6 py-4 border-t border-slate-200">
                <div class="flex items-center justify-between">
                    <div class="text-sm text-slate-600">Menampilkan halaman {{ $services->currentPage() }} dari {{ $services->lastPage() }}</div>
                    <div>
        ...
        @else
            <div class="px-6 py-10 text-center text-slate-500"><p class="text-lg">Tidak ada layanan.</p></div>
        @endif
    </div>
</div>

<script>
function deleteService(serviceId, baseIndexUrl) {
    if (!confirm('Hapus layanan ini?')) return;

    const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
    const deleteUrl = baseIndexUrl + '/' + serviceId; // endpoint: /admin/services/{id} atau /editor/services/{id}
    ...
    .then(data => {
        if (data.success) {
            ...
            msg.textContent = 'Layanan berhasil dihapus';
