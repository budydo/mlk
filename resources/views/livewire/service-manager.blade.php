...
{{-- DEPRECATED: Modul Services sudah dipindahkan ke implementasi server-side (non-Livewire).
     Jangan gunakan file ini lagi; gunakan route /admin/services atau /editor/services untuk akses. --}}
<div class="py-10">
    <div class="max-w-6xl mx-auto px-4 sm:px-6">
        <div class="mb-6">
            <div class="bg-yellow-50 border border-yellow-200 text-yellow-800 px-4 py-3 rounded">Perhatian: Livewire ServiceManager tidak lagi digunakan. Gunakan halaman Admin/Editor Services (non-Livewire).</div>
        </div>
        ...
        {{-- Filter: Checkbox untuk menampilkan hanya layanan yang dipublikasikan --}}
        <div class="mb-6 flex items-center gap-4">
            {{-- Checkbox ini terhubung ke property Livewire `onlyPublished` --}}
            <label class="flex items-center text-sm text-slate-700">
                <input
                    type="checkbox"
                    wire:model="onlyPublished"
                    class="w-4 h-4 text-emerald-600 rounded"
                >
                <span class="ml-2">Tampilkan hanya yang dipublikasikan</span>
            </label>
            {{-- Informasi singkat: jika diaktifkan, tabel hanya menampilkan is_published = 1 --}}
            <p class="text-xs text-slate-500">(Filter ini hanya menampilkan layanan dengan <code>is_published = 1</code>)</p>
        </div>
        ...
        @if($this->showForm)
            <div class="bg-white rounded-lg shadow p-6 mb-6">
                ...
                <form wire:submit="save" class="space-y-4">
                    ...
                    {{-- Upload Gambar Layanan atau Link Eksternal --}}
                    <div>
                        ...
                        @foreach($services as $service)
                            <tr class="hover:bg-slate-50 transition">
                                ...
                                <td class="px-6 py-4 text-sm">
                                        {{-- Toggle publikasi langsung dari tabel: checkbox memanggil method Livewire togglePublished --}}
                                        <label class="inline-flex items-center cursor-pointer">
                                            {{-- Checkbox berubah saat diklik, memanggil togglePublished dengan id service --}}
                                            <input
                                                type="checkbox"
                                                wire:click="togglePublished({{ $service->id }})"
                                                class="form-checkbox h-4 w-4 text-emerald-600"
                                                {{-- Checked jika service dipublikasikan --}}
                                                @if($service->is_published) checked @endif
                                            >
                                            {{-- Teks status; tampilkan label singkat sesuai nilai saat ini --}}
