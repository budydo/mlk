  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      @forelse($services->take(9) as $service)
        <a href="{{ route('services.show', $service->slug) }}" class="block bg-white rounded-xl overflow-hidden shadow hover:shadow-lg transition">
          @php
            $serviceImage = $service->image_path ?? $service->cover_image ?? null;
          @endphp
          @if($serviceImage)
            <img src="{{ imageUrl($serviceImage) }}" alt="{{ $service->title }}" class="w-full h-60 object-cover" />
          @else
